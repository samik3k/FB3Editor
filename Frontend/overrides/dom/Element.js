	...
	'FBEditor.override.dom.Element',
	{
		...
		privates: {
			doAddListener: function (eventName, fn, scope, options, order, caller, manager)
			{
				...
				if (eventName === 'click'){console.log(3, '> Ext.dom.Element#doAddListener', eventName, me.blockedEvents, me);}
				...
				if (!me.blockedEvents[eventName])
				{
					...
					options = options || {};
					if (eventName === 'click'){console.log(3.1, options, observableDoAddListener);}

					if (me.longpressEvents[eventName])
					{
						if (eventName === 'click'){console.log(3.12);}
					...
					if (Element.useDelegatedEvents === false)
					{
						...
						if (eventName === 'click'){console.log(3.13, options.delegated);}
					}
					...
					if (options.translate !== false)
					{
						...
						if (eventName === 'click'){console.log(3.14, additiveEventName, me.additiveEvents);}
						if (additiveEventName)
						{
							if (eventName === 'click'){console.log(3.141, additiveEventName);}
						...
						{
							if (eventName === 'click'){console.log(3.142, translatedEventName);}
