...
Ext.define(
	'FBEditor.override.Component',
	{
		...
		addManagedListener: function(item, ename, fn, scope, options, /* private */ noDestroy) {
			...
			if (ename === 'click' || ename.click || ename === 'tap' || ename.tap){console.log(2, '> Ext.mixin.Observable#addManagedListener', item);}
			...
			if (typeof ename !== 'string') {
				...
				if (ename === 'click' || ename.click || ename === 'tap' || ename.tap){console.log(2.1, passedOptions);}

				options = ename;
				for (ename in options) {
					if (options.hasOwnProperty(ename)) {
						config = options[ename];
						if (ename === 'click' || ename.click || ename === 'tap' || ename.tap){console.log(2.11, config, item.$eventOptions);}
						if (!item.$eventOptions[ename]) {
							if (ename === 'click' || ename.click || ename === 'tap' || ename.tap){console.log(2.111);}
				...
				if (options && options.destroyable) {
					if (ename === 'click' || ename.click || ename === 'tap' || ename.tap){console.log(2.12, passedOptions);}
					return new ListenerRemover(me, item, options);
				}
			}
			else {
				if (ename === 'click' || ename === 'tap'){console.log(2.2, fn);}
				if (fn !== Ext.emptyFn) {
					if (ename === 'click' || ename === 'tap'){console.log(2.21, options);}
					...
					if (!noDestroy && options && options.destroyable) {
						if (ename === 'click' || ename === 'tap'){console.log(2.211);}
